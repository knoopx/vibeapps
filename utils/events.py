#!/usr/bin/env python3
"""
Evolution Data Server Calendar Events Lister

This script connects to Evolution Data Server and lists calendar events.
Requires: python3-gi, gir1.2-ecal-2.0, gir1.2-edataserver-1.2

Install dependencies on Ubuntu/Debian:
sudo apt-get install python3-gi gir1.2-ecal-2.0 gir1.2-edataserver-1.2
"""
...
class EDSCalendarLister:
    ...
    def get_calendar_client(self, source):
        """Create calendar client for a source"""
        try:
            # Check if this is a contacts-based calendar
            backend_name = None
            try:
            ...
            # Try different approaches for different EDS versions
            client = None

            # Method 1: Try connect_sync with proper timeout (newer API)
            if hasattr(ECal.Client, 'connect_sync'):
                try:
                    # Use 30 second timeout instead of -1
                    client = ECal.Client.connect_sync(source, ECal.ClientSourceType.EVENTS, 30, None)
                    if client:
                        # Store backend info for later use
                        client._backend_name = backend_name
                        return client
                except Exception as e:
                    print(f"connect_sync failed: {e}")

            # Method 2: Try the new() + open_sync() pattern (older API)
            ...
            # Method 3: Try without timeout parameter
            if hasattr(ECal.Client, 'connect_sync'):
                try:
    ...
    def list_events(self, client, start_date=None, end_date=None):
        """List events from a calendar client"""
        if not start_date:
        ...
        try:
            ...
            # Method 1: Try newer EDS query format (ISO format)
            try:
                ...
                # Handle tuple result (success, objects)
                if isinstance(result, tuple) and len(result) >= 2:
                    ...
                    if success and objects:
                        ...
                        if events:
                            return events
                elif result:  # Direct objects list
            ...
            # Method 2: Try EDS native time format (YYYYMMDDTHHMMSSZ)
            try:
            ...
            # Method 3: Try without time range (get all events, then filter)
            try:
                query = "#t"  # Get all objects
                result = client.get_object_list_sync(query, None)
                ...
                if objects:
                    ...
                    for event in all_events:
                        if (event.get('start_time') and
                            start_date <= event['start_time'] <= end_date):
                            events.append(event)
                        elif not event.get('start_time'):  # Include events without dates
                            events.append(event)
                    return events
            except Exception as e:
    ...
    def process_objects(self, objects):
        """Process calendar objects into event list"""
        events = []

        if not objects:
            return events

        # Handle case where objects might be a single object or a list
        ...
        for obj in objects:
            try:
                ...
                if component:
                    ...
                    # Try to get basic info even if we can't extract component
    ...
    def extract_basic_info(self, obj):
        """Extract basic info from objects that don't have standard component interface"""
        try:
            ...
            # Only return if we got at least a UID or summary
    ...
    def extract_component(self, obj):
        """Extract calendar component from various object types"""
        if obj is None:
            return None
        ...
        if hasattr(obj, 'as_string'):
            try:
                # This would need iCal parsing, skip for now
                return None
            except:
                return None
    ...
    def parse_event_component(self, component):
        """Parse event component and extract information"""
        try:
            # Handle different component types
            if component is None:
                return None

            # Skip non-component objects (bool, list, etc.)
            if isinstance(component, (bool, list, str, int)):
                return None

            # Check if it's actually a calendar component
            if not hasattr(component, 'get_uid') and not hasattr(component, 'get_summary'):
                # Try to extract from different container types
            ...
            try:
                ...
                if dtstart:
                    ...
                    if dtstart and hasattr(dtstart, 'get_timezone'):
                        event['all_day'] = dtstart.get_timezone() is None and (
                            not hasattr(dtstart, 'get_hour') or dtstart.get_hour() == 0
                        )
            except:
                pass
    ...
    def ical_time_to_datetime(self, ical_time):
        """Convert iCal time to Python datetime"""
        if not ical_time:
            return None
        ...
        try:
            return datetime(
                ...
                ical_time.get_day(),
                ical_time.get_hour(),
                ical_time.get_minute(),
                ical_time.get_second()
            )
        except:
            return None
    ...
    def list_contact_events(self, client, start_date, end_date):
        """Special handling for contact-based calendars (birthdays/anniversaries)"""
        events = []

        try:
            # For contact-based calendars, we need to try different approaches
            print("Trying contact-based calendar methods...")

            # Method 1: Try to get recurring events with a wider time range
            try:
                # Birthdays repeat yearly, so we need to look for recurring events
                # Expand the time range to catch recurring events
            ...
            # Method 2: Try to get objects with recurrence rules
            try:
            ...
            # Method 3: Try to access the underlying address book
            try:
                print("Trying to access underlying address book...")

                # Get the source registry to find address books
                registry = client.get_registry() if hasattr(client, 'get_registry') else self.registry
                if registry:
                    # Look for address book sources
    ...
    def extract_birthdays_from_addressbook(self, book_source, start_date, end_date):
        """Extract birthday/anniversary events from an address book"""
        events = []

        try:
            # Import EBook for address book access
            gi.require_version('EBook', '1.2')
            from gi.repository import EBook

            # Create address book client
            book_client = None
    ...
    def extract_contact_dates(self, contact, current_year, start_date, end_date):
        """Extract birthday and anniversary dates from a contact"""
        events = []
        ...
        try:
                        ...
                        for year in range(current_year - 1, current_year + 2):
                            try:
                                ...
                                if start_date <= birthday <= end_date:
                                    ...
                                    events.append({
                                        'uid': f"birthday-{contact.get_uid()}-{year}",
                                        'summary': summary,
            ...
            # Try to get anniversary (this might be in custom fields)
            # Wedding anniversaries are often stored differently
    ...
    for cal in calendars:
        ...
        if not events:
                    ...
                    if objects:
                        ...
                        if hasattr(objects, '__iter__') and len(objects) > 0:
                            ...
                            for i, obj in enumerate(objects[:3]):  # Show first 3
                                ...
                                if obj_info:
                                    for info in obj_info[:2]:  # Show first 2 pieces of info
                                        print(f"    {info}")
                                else:
                                    print(f"    No accessible attributes found")
                                    # Show available methods/attributes
            ...
            if not events:
                ...
                # First, let's get more info about the calendar itself
                ...
                # Method 1: Try to get any components at all
                try:
                    result = client.get_object_list_sync("", None)  # Empty query
                    if isinstance(result, tuple) and len(result) >= 2:
                        success, objects = result[0], result[1]
                        if success and objects:
                ...
                # Method 2: Try different query syntaxes
                if not events:
                    query_attempts = [
                        "(contains? \"any\" \"\")",
                        "(= \"any\" \"\")",
                        "(exists? \"uid\")",
                        "(has-categories? #f)",
                        "(not (has-categories? #t))",
                        "#f",  # Get nothing (inverse of #t)
                    ]
                ...
                # Method 3: Try using the client in different ways
                if not events:
                    ...
                    try:
                        ...
                        # Try some alternative data access methods
                        ...
                        if not events and hasattr(client, 'get_view'):
                            try:
                                ...
                                if view:
                                    ...
                                    # Views usually need to be started
                                    if hasattr(view, 'start'):
                                        view.start()
                                        # This is async, but let's see if we can get anything
                                        import time
                                        time.sleep(0.1)  # Brief wait

                                    # Try to get objects from view
            ...
            # Special handling for contact-based calendars
        ...
        try:
            ...
            pass  # Some clients don't need explicit closing
