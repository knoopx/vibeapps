...
class CollectionsManager:
    ...
    def _load_all_collections(self) -> None:
        ...
        for file_path in self.base_dir.glob("*.json"):
            self._collections[file_path.stem] = CollectionManager(file_path)

    def _load_collection(self, name: str) -> CollectionManager:
        if name not in self._collections:
            collection_file = self.base_dir / f"{name}.json"
            self._collections[name] = CollectionManager(collection_file)
        return self._collections[name]

    def keys(self) -> Set[str]:
        return set(self._collections.keys())

    def __getitem__(self, name: str) -> CollectionManager:
        return self._load_collection(name)

    def __setitem__(self, name: str, collection: CollectionManager) -> None:
    ...
    def values(self) -> ValuesView[CollectionManager]:
        return self._collections.values()

    def items(self) -> ItemsView[str, CollectionManager]:
        return self._collections.items()

    def get(
        self, name: str, default: Optional[CollectionManager] = None
    ...
    def lookup(self, release_path: str) -> list[CollectionManager]:
        return [
            collection
