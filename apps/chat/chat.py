...
class ChatAppWindow(Gtk.ApplicationWindow):
    ...
    def _handle_height_result(self, web_view, async_result, user_data):
        try:
            ...
            pass    def update_webview_height(self, web_view, height):
        webview_id = id(web_view)
        current_height = self.webview_heights.get(webview_id, 0)

        # Ensure height can only grow
        if height > current_height:
            height_val = height
            self.webview_heights[webview_id] = height
