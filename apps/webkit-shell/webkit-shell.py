...
class WebKitShell(Gtk.Application):
    ...
    def __init__(self, app_id=APP_ID, url=None, width=800, height=600, title='WebKit Shell'):
        super().__init__(application_id=app_id, flags=Gio.ApplicationFlags.HANDLES_COMMAND_LINE | Gio.ApplicationFlags.REPLACE)
        self.url = url
    ...
    def do_activate(self):
        ...
        if not self.window:
            ...
            if self.url:
                ...
                self.webview.load_html('<h1>No URL Provided</h1><p>Please launch with a URL, e.g., <code>python webkit-shell.py https://www.google.com</code> or <code>python webkit-shell.py --url https://www.google.com</code></p>', 'file:///')
...
def main():
    ...
    parser.add_argument('--app-id', '-a', default=APP_ID, help='Application ID for the GTK application')
    ...
    app = WebKitShell(app_id=args.app_id, url=args.url, width=args.width, height=args.height, title=args.title)
