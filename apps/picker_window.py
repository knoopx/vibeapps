...
class PickerWindow(Adw.ApplicationWindow, ABC, metaclass=GObjectABCMeta):
    ...
    def __init__(self, title: str='Picker', search_placeholder: str='Search...', window_size: tuple=(500, 900), search_delay_ms: int=300, enable_context_menu: bool=True, context_menu_shortcut: str='<Control>j', **kwargs):
    ...
    def _setup_ui(self):
        ...
        self._search_entry = Gtk.SearchEntry(hexpand=True, placeholder_text=self._search_placeholder)
    ...
    def _setup_status_pages(self):
        loading_page = Adw.StatusPage(title='Loading...', icon_name=self.get_loading_icon())
        ...
        self._empty_page = Adw.StatusPage(title=self.get_empty_title(), description=self.get_empty_description(), icon_name=self.get_empty_icon())
        self._content_stack.add_named(self._empty_page, 'empty')
        self._error_page = Adw.StatusPage(title='An Error Occurred', description='Could not load data.', icon_name='dialog-error-symbolic')
    ...
    def _setup_signals(self):
        ...
        if self._enable_context_menu:
            ...
            context_menu_shortcut = Gtk.Shortcut.new(Gtk.ShortcutTrigger.parse_string(self._context_menu_shortcut), Gtk.CallbackAction.new(self._show_context_menu_action_callback))
    ...
    def _on_search_changed(self, entry):
        ...
        if not query:
            ...
            return
        self._search_delay_id = GLib.timeout_add(self._search_delay_ms, self._apply_search, query)
