...
class ScratchpadWindow(Adw.ApplicationWindow):
    ...
    def _is_error_result(self, result_text, line_type):
        if not result_text or line_type is None:
            return False
        error_patterns = ['Division by zero', 'Undefined variable', 'Invalid syntax', 'Invalid value', 'NameError:', 'SyntaxError:', 'ValueError:', 'ZeroDivisionError:', 'TypeError:']
...
class ScratchpadApplication(Adw.Application):
    ...
    def __init__(self):
        super().__init__(application_id='net.knoopx.scratchpad', flags=Gio.ApplicationFlags.FLAGS_NONE)
